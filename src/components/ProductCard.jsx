...
const ProductCard = ({ product, addToCart }) => {
  ...
  const handleAddToCart = () => {
    ...
    // For footwear, require size selection
    if (product.category === "footwear" && !selectedSize) {
      alert(`Please select a size for ${product.name}`);
      return;
    }
    
    // For footwear, check size availability
    if (product.category === "footwear" && selectedSize && product.availableSizes) {
              ...
              {product.sizes.map(size => (
                <button
                  ...
                  className={`size-option ${selectedSize === size ? 'selected' : ''} ${
                    product.availableSizes && product.availableSizes[size] <= 0 ? 'out-of-stock' : ''
                  }`}
                  onClick={() => setSelectedSize(size)}
                  disabled={product.availableSizes && product.availableSizes[size] <= 0}
                  title={product.availableSizes && product.availableSizes[size] <= 0 ? 'Out of stock' : `Select size ${size}`}
                >
                  {size}
                  {product.availableSizes && product.availableSizes[size] <= 0 && (
                    <span className="size-stock-badge">X</span>
                  )}
                </button>
              ))}
            </div>
        ...
        <div className="quantity-selector">
          ...
          <button
            className={`add-to-cart-btn ${product.stock === 0 ? 'out-of-stock' : ''} ${
              product.category === "footwear" && !selectedSize ? 'disabled' : ''
            }`}
            onClick={handleAddToCart}
            disabled={product.stock === 0 || (product.category === "footwear" && !selectedSize)}
            title={
              product.stock === 0 ? 'Out of stock' : 
              product.category === "footwear" && !selectedSize ? 'Select a size first' : 
              'Add to cart'
            }
          >
            {product.stock === 0 ? 'Out of Stock' : 
             product.category === "footwear" && !selectedSize ? 'Select Size' : 
             'Add to Cart'}
          </button>
        </div>
