...
export const createOrder = async (orderData, cartItems) => {
  try {
    ...
    /* NEW CODE - Record sale in expense ledger
    Now that the order and its items are successfully created, we record
    the sale as a "sale" type entry in the expenses ledger.
    This gives us a single source of truth for all financial data:
    - "sale" entries = money IN (revenue)
    - "product_purchase" / "restock" entries = money OUT (expenses)
    The amount is stored at the current sale price — never recalculated —
    so it is safe against future price changes.
    The created_at timestamp enables accurate time-based filtering
    (today, last 7 days, last 30 days) across all financial views.
    */
    try {
      const itemSummary = cartItems
        .map((item) => `${item.quantity}x ${item.name || `Product #${item.id}`}`)
        .join(", ");

      await addExpense({
        type: "sale",
        description: `Order ${orderRecord.order_number} — ${itemSummary}`,
        amount: totals.total,
        created_at: new Date().toISOString(),
      });

      console.log(
        `✅ Sale recorded in expense ledger for order ${orderRecord.order_number}: $${totals.total}`,
      );
    } catch (expenseError) {
      // Don't throw — order was created successfully, just log the warning
      console.warn(
        "⚠️ Order created but sale entry could not be recorded in ledger:",
        expenseError,
      );
    }
