...
const AdminDashboard = ({ user }) => {
  ...
  */
  ...
  // Stats functions
  /* NEW CODE - calculateStats now uses the expense ledger as a single source of truth
  for all financial data. This approach is:
  1. Price-change safe: amounts are stored at transaction time, never recalculated
  2. Timestamp accurate: created_at on each expense row enables precise time filtering
  3. Unified: revenue (sales) and costs (purchases/restocks) all live in one table

  Ledger entry types:
    "sale"             → money IN  (recorded by createOrder in orderService.js)
    "product_purchase" → money OUT (recorded by handleAddProduct)
    "restock"          → money OUT (recorded by handleRestock)

  todaySales uses created_at with proper midnight boundaries so it correctly
  captures all orders placed today regardless of timezone string formatting.
  */
  const calculateStats = (products, orders, expensesList) => {
    const allExpenses = expensesList || [];

    // Total earned = sum of all "sale" ledger entries
    const totalEarned = allExpenses
      .filter((e) => e.type === "sale")
      .reduce((sum, e) => sum + e.amount, 0);

    // Total expenses = sum of all non-sale ledger entries (purchases + restocks)
    const totalExpenses = allExpenses
      .filter((e) => e.type !== "sale")
      .reduce((sum, e) => sum + e.amount, 0);

    // Net profit = money in minus money out
    const netProfit = totalEarned - totalExpenses;

    // Today's sales: filter "sale" entries by created_at timestamp using
    // proper midnight boundaries — safe against date string formatting differences
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const todayEnd = new Date();
    todayEnd.setHours(23, 59, 59, 999);

    const todaySales = allExpenses
      .filter((e) => {
    ...
    // Gross profit kept for the profit modal's "Most Profitable Products" section
    const cogs = orders.reduce((sum, order) => {
      return (
        sum +
        order.items.reduce((itemSum, item) => {
  ...
  */
  ...
  /* NEW CODE - filterExpensesByPeriod
  Filters the expense ledger by created_at timestamp for the given period.
  Used by the Revenue modal and Profit modal to show time-filtered financial data.
  Since all financial events (sales, purchases, restocks) now have a real
  created_at timestamp, this single helper drives all time-based financial views.
  */
  const filterExpensesByPeriod = (expensesList, filterType) => {
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const todayEnd = new Date();
    todayEnd.setHours(23, 59, 59, 999);
    ...
    return expensesList.filter((e) => {
      ...
      if (filterType === "7days") {
        ...
        start.setHours(0, 0, 0, 0);
        return entryDate >= start && entryDate <= todayEnd;
      }
              ...
              >
                {["today", "7days", "30days", "all"].map((f) => (
                  <button
                    ...
                    style={{
                      padding: "8px 12px",
                      backgroundColor: revenueFilter === f ? "#3498db" : "#ecf0f1",
                      color: revenueFilter === f ? "white" : "#333",
                      border: "none",
                      borderRadius: "4px",
                      cursor: "pointer",
                      fontWeight: revenueFilter === f ? "bold" : "normal",
                    }}
                  >
                    {f === "today" ? "Today" : f === "7days" ? "Last 7 Days" : f === "30days" ? "Last 30 Days" : "All Time"}
                  </button>
                ))}
              </div>

              {/* NEW CODE - Revenue modal stats now driven by filterExpensesByPeriod
              so the numbers update correctly when the admin switches time filters.
              totalEarned for the period = sum of "sale" entries in that window.
              avgOrderValue = filtered earned / count of sale entries in that window.
              */}
              {(() => {
                const periodExpenses = filterExpensesByPeriod(expenses, revenueFilter);
                const periodSales = periodExpenses.filter((e) => e.type === "sale");
                const periodEarned = periodSales.reduce((sum, e) => sum + e.amount, 0);
                const periodOrderCount = periodSales.length;
                const avgOrderValue = periodOrderCount ? periodEarned / periodOrderCount : 0;

                return (
                  <div className="stats-detail">
                    <div className="detail-item">
                      <label>Total Earned</label>
                      <span className="value">{formatCurrency(periodEarned)}</span>
                    </div>
                    <div className="detail-item">
                      <label>Today's Sales</label>
                    ...
                    <div className="detail-item">
                      <label>Avg. Order Value</label>
                      <span className="value">{formatCurrency(avgOrderValue)}</span>
                    </div>
                    <div className="detail-item">
                      <label>Orders in Period</label>
                      <span className="value">{periodOrderCount}</span>
                    </div>
                  </div>
                );
              })()}
        ...
        <div className="modal-overlay">
          ...
          >
            ...
            <div className="modal-body">
              ...
              >
                {["today", "7days", "30days", "all"].map((f) => (
                  <button
                    ...
                    style={{
                      padding: "8px 12px",
                      backgroundColor: profitFilter === f ? "#3498db" : "#ecf0f1",
                      color: profitFilter === f ? "white" : "#333",
                      border: "none",
                      borderRadius: "4px",
                      cursor: "pointer",
                      fontWeight: profitFilter === f ? "bold" : "normal",
                    }}
                  >
                    {f === "today" ? "Today" : f === "7days" ? "Last 7 Days" : f === "30days" ? "Last 30 Days" : "All Time"}
                  </button>
              ...
              {/* NEW CODE - Profit modal stats now driven by filterExpensesByPeriod
              so earned, expenses, net profit, and margin all update with the time filter.
              "sale" entries = earned for that period.
              non-"sale" entries = expenses for that period.
              net = earned - expenses for that same window.
              */}
              {(() => {
                const periodExpenses = filterExpensesByPeriod(expenses, profitFilter);
                const periodEarned = periodExpenses
                  .filter((e) => e.type === "sale")
                  .reduce((sum, e) => sum + e.amount, 0);
                const periodCosts = periodExpenses
                  .filter((e) => e.type !== "sale")
                  .reduce((sum, e) => sum + e.amount, 0);
                const periodNet = periodEarned - periodCosts;
                const periodNetPositive = periodNet >= 0;
                const margin = periodEarned > 0
                  ? ((periodNet / periodEarned) * 100).toFixed(1)
                  : 0;
                ...
                return (
                  <>
                    <div className="stats-detail">
                      ...
                      <div className="detail-item">
                        <label>Total Expenses</label>
                        <span className="value loss">{formatCurrency(periodCosts)}</span>
                      </div>
                      <div className="detail-item">
                        <label>Net {periodNetPositive ? "Profit" : "Loss"}</label>
                        <span className={`value ${periodNetPositive ? "profit" : "loss"}`}>
                          {periodNetPositive ? "+" : "-"}{formatCurrency(Math.abs(periodNet))}
                        </span>
                      </div>
                      <div className="detail-item">
                        <label>Profit Margin</label>
                    ...
                    <div className="pnl-summary">
                      <div className="pnl-row earned">
                        <span>💵 Money Earned</span>
                        <span>{formatCurrency(periodEarned)}</span>
                      </div>
                      <div className="pnl-row loss">
                        <span>💸 Money Spent</span>
                        <span>- {formatCurrency(periodCosts)}</span>
                      </div>
                      <div className={`pnl-row ${periodNetPositive ? "net-profit" : "net-loss"}`}>
                        <span><strong>= Net {periodNetPositive ? "Profit" : "Loss"}</strong></span>
                        <span>
                          <strong>
                            {periodNetPositive ? "+" : "-"}{formatCurrency(Math.abs(periodNet))}
                          </strong>
        ...
        <div className="modal-overlay">
          ...
          >
            ...
            <div className="modal-body">
              <div className="financial-summary">
                ...
                <div className="fin-card expenses">
                  ...
                  <div>
                    ...
                    <p className="fin-sub">Product costs & restocks</p>
                  </div>
                </div>
                <div
                  className={`fin-card ${isNetPositive ? "profit" : "loss"}`}
              ...
              {/* NEW CODE - Expense ledger now shows all entry types with clear labels.
              "sale" entries show as revenue (money IN) in green.
              "product_purchase" and "restock" entries show as costs (money OUT) in red.
              The created_at timestamp is displayed so the admin can see exactly
              when each financial event occurred, enabling accurate period analysis.
              */}
              <h4>📋 Full Financial Ledger</h4>
              <table className="modal-table">
                <thead>
                  <tr>
                    <th>Date & Time</th>
                ...
                <tbody>
                  {expenses.length === 0 ? (
                    ...
                    expenses.map((exp) => (
                      <tr key={exp.id}>
                        <td>
                          {new Date(exp.created_at || exp.date).toLocaleString()}
                        </td>
                        <td>
                          <span
                        ...
                        <td
                          className={
                            exp.type === "sale" ? "profit-text" : "loss-text"
                          }
                        >
                          {exp.type === "sale" ? "+" : "-"}
                          {formatCurrency(exp.amount)}
                ...
                <tfoot>
                  <tr>
                    ...
                    <td className={isNetPositive ? "profit-text" : "loss-text"}>
                      <strong>
                        {isNetPositive ? "+" : "-"}
                        {formatCurrency(Math.abs(stats.netProfit))}
                      </strong>
