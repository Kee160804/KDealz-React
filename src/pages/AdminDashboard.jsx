...
//     return {
//       ...rawProduct,
//       stock_quantity: rawProduct.stock_quantity,          // alias for ProductCard compatibility
//       sizes,
//       availableSizes,
//     };
//   };
...
//         ...p,
//         alertLevel:
//           p.stock_quantity < 5 ? "critical" : p.stock_quantity < 10 ? "warning" : "notice",
//       }))
...
//         amount: parseFloat(newProduct.cost) * parseInt(newProduct.stock_quantity),
//         productId: id,
//       };
...
//       const updatedProduct = await updateStockMutation.mutateAsync({ id: productId, newStock: product.stock_quantity + amount });
//       const updatedProducts = products.map((p) =>
...
//                         </td>
//                         <td className={product.stock_quantity < 10 ? "stock-low" : ""}>
...
//                   <span className={selectedItem.stock_quantity < 10 ? "stock-low" : ""}>
...
//                 <div className="detail-row">
//                   <label>Current Stock:</label>
//                   <span className="stock-low">{selectedItem.stock_quantity} units</span>
//                 </div>
//                 <div className="detail-row">
...
//                       setNewProduct({
//                         ...newProduct,
//                         sizes: [...newProduct.sizes, { sizes: "", stock_quantity: 0 }]
//                       });
//                     }}
//                   >
//                     + Add Size
...
//                         parseFloat(newProduct.cost || 0) * parseInt(newProduct.stock_quantity || 0)
//                       )}
//                     </div>
//                     <div>
//                       Expected Revenue: {formatCurrency(
//                         parseFloat(newProduct.price || 0) * parseInt(newProduct.stock_quantity || 0)
//                       )}
//                     </div>
//                     <div>
...
//               </p>
//               <p>
//                 <strong>Current Stock:</strong> {selectedItem.stock_quantity} units
//               </p>
//               <p>
...
const transformProduct = (rawProduct) => {
  ...
  const handleAddProduct = async () => {
    try {
      ...
      image: newProduct.image || "https://via.placeholder.com/500",
      sizes: sizesArray.length > 0 ? JSON.stringify(sizesArray) : null,
      available_Sizes: sizesArray.length > 0 ? JSON.stringify(availableSizesObj) : null, // ✅ capital S
    };
  ...
  const handleEditProduct = async () => {
    try {
      ...
      console.log("Updating product ID", selectedItem.id, "with payload:", updatedData);

      await updateProductMutation.mutateAsync({ id: selectedItem.id, data: updatedData });
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className={`mobile-nav-menu ${isMobileMenuOpen ? "active" : ""}`}>
        ...
        <div className="mobile-nav-items">
          <button onClick={() => { openModal("products"); setIsMobileMenuOpen(false); }}>📦 Products</button>
          <button onClick={() => { openModal("orders"); setIsMobileMenuOpen(false); }}>🛒 Orders</button>
          <button onClick={() => { openModal("users"); setIsMobileMenuOpen(false); }}>👥 Users</button>
          <button onClick={() => { openModal("revenue"); setIsMobileMenuOpen(false); }}>💵 Revenue</button>
          <button onClick={() => { openModal("financial"); setIsMobileMenuOpen(false); }}>💸 Expenses</button>
          <button onClick={() => { openModal("profit"); setIsMobileMenuOpen(false); }}>📈 Profit/Loss</button>
          <button onClick={() => { openModal("inventory"); setIsMobileMenuOpen(false); }}>⚠️ Inventory</button>
          <button onClick={() => { openModal("pending"); setIsMobileMenuOpen(false); }}>⏳ Pending</button>
          <button onClick={() => { openModal("sales"); setIsMobileMenuOpen(false); }}>📊 Sales</button>
        </div>
      </div>
      ...
      <div className="stats-grid">
        <div className="stat-card" onClick={() => openModal("products")}>
          ...
          <div className="stat-content">
            <h3>Total Products</h3>
            <p className="stat-value">{stats.totalProducts}</p>
            <span className="stat-link">View <span className="arrow">→</span></span>
          </div>
        </div>
        <div className="stat-card" onClick={() => openModal("orders")}>
          <div className="stat-icon">🛒</div>
      ...
      {inventoryAlerts.length > 0 && (
        <div className="alerts-section">
          ...
          <div className="alerts-grid">
            {inventoryAlerts.map((product) => (
              <div key={product.id} className={`alert-card ${product.alertLevel}`} onClick={() => openDetailModal("inventoryDetail", product, "inventory")}>
                <div className="alert-header">
                  <h4>{product.name}</h4>
                  <span className="alert-badge">{product.alertLevel}</span>
                </div>
                <p>SKU: {product.SKU || `SKU-${product.id}`}</p>
                <div className="stock-bar">
                  <div className="stock-fill" style={{ width: `${Math.min((product.stock_quantity / 20) * 100, 100)}%`, backgroundColor: product.alertLevel === "critical" ? "#e74c3c" : product.alertLevel === "warning" ? "#f39c12" : "#3498db" }} />
                </div>
                <p className="stock-count">Current Stock: <strong>{product.stock_quantity} units</strong></p>
                <button className="restock-btn" onClick={(e) => { e.stopPropagation(); openDetailModal("restock", product, "inventory"); }}>Restock Now →</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="quick-actions-section">
        <h3>⚡ Quick Actions</h3>
        <div className="action-buttons-grid">
          <button className="action-btn" onClick={() => setShowAddProductModal(true)}><span className="action-icon">➕</span><span className="action-text">Add Product</span></button>
          <button className="action-btn" onClick={() => openModal("inventory")}><span className="action-icon">📦</span><span className="action-text">Update Stock</span></button>
          <button className="action-btn" onClick={() => openModal("pending")}><span className="action-icon">✅</span><span className="action-text">Process Orders</span></button>
          <button className="action-btn" onClick={() => openModal("financial")}><span className="action-icon">💰</span><span className="action-text">Financial Report</span></button>
        </div>
      </div>
      ...
      <div className="recent-orders-section">
        <div className="section-header">
          <h3>📋 Recent Orders</h3>
          <button className="view-all-btn" onClick={() => openModal("orders")}>View All →</button>
        </div>
        <div className="orders-table-container">
          <table className="orders-table">
            <thead>
            ...
            <tbody>
              {recentOrders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td><button className="table-action-btn" onClick={(e) => { e.stopPropagation(); openDetailModal("orderDetail", order, "orders"); }}>View</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showProductModal && (
        <div className="modal-overlay">
          <div className="modal-content large" ref={modalRefs.product} tabIndex="-1" role="dialog" aria-modal="true" aria-label="All Products">
            <div className="modal-header">
              <h2>📦 All Products</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
                ...
                <tbody>
                  {products.map((product) => (
                    <tr key={product.id}>
                      ...
                      <td className={product.stock_quantity < 10 ? "stock-low" : ""}>{product.stock_quantity}</td>
                      <td>{product.SKU || `SKU-${product.id}`}</td>
                      <td className="action-cell">
                        <button className="action-small view" onClick={() => openDetailModal("productDetail", product, "product")}>View</button>
                        <button className="action-small edit" onClick={() => openEditModal(product, "product")}>Edit</button>
                        <button className="action-small delete" onClick={() => handleDeleteProduct(product.id)}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="modal-footer">
              <button className="btn-primary" onClick={() => { closeAllModals(); setShowAddProductModal(true); }}>➕ Add New Product</button>
      ...
      {showOrderModal && (
        <div className="modal-overlay">
          <div className="modal-content large" ref={modalRefs.order} tabIndex="-1" role="dialog" aria-modal="true" aria-label="All Orders">
            <div className="modal-header">
              <h2>🛒 All Orders</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
            <div className="order-filters">
              <div className="filters-container">
                <div className="search-box">
                  <span className="search-icon">🔍</span>
                  <input type="text" placeholder="Search by name, email, or order ID..." value={orderSearchTerm} onChange={(e) => setOrderSearchTerm(e.target.value)} className="search-input" />
                  {orderSearchTerm && <button className="clear-search" onClick={() => setOrderSearchTerm("")}>✕</button>}
                </div>
                <div className="filter-dropdown">
                  <select value={orderFilterStatus} onChange={(e) => setOrderFilterStatus(e.target.value)} className="category-select">
                    <option value="all">All Statuses</option>
                    <option value="pending">⏳ Pending</option>
                    <option value="processing">⚙️ Processing</option>
                    <option value="shipped">🚚 Shipped</option>
                ...
                <button className="date-filter-btn" onClick={() => setShowDatePicker(!showDatePicker)}>
                  <span>📅</span> {startDate || endDate ? "Date Filter Active" : "Filter by Date"}
                </button>
                {(orderSearchTerm || orderFilterStatus !== "all" || startDate || endDate) && (
                  <button className="clear-filters-btn" onClick={() => { setOrderSearchTerm(""); setOrderFilterStatus("all"); setStartDate(""); setEndDate(""); setShowDatePicker(false); }}>✕ Clear Filters</button>
                )}
              </div>
              {showDatePicker && (
                <div className="date-picker-container">
            ...
            <div className="modal-body">
              <table className="modal-table">
                ...
                <tbody>
                  {orders.filter((order) => {
                    const searchLower = orderSearchTerm.toLowerCase();
                    const matchesSearch = orderSearchTerm === "" || order.customer?.toLowerCase().includes(searchLower) || order.email?.toLowerCase().includes(searchLower) || order.id?.toString().includes(searchLower);
                    const matchesStatus = orderFilterStatus === "all" || order.status === orderFilterStatus;
                    let matchesDate = true;
                    if (startDate || endDate) {
                      const orderDate = new Date(order.date);
                      orderDate.setHours(0,0,0,0);
                      if (startDate) { const start = new Date(startDate); start.setHours(0,0,0,0); matchesDate = matchesDate && orderDate >= start; }
                      if (endDate) { const end = new Date(endDate); end.setHours(23,59,59,999); matchesDate = matchesDate && orderDate <= end; }
                    }
                    return matchesSearch && matchesStatus && matchesDate;
                  }).map((order) => (
                    <tr key={order.id}>
                      ...
                      <td className="order-total">{formatCurrency(order.total)}</td>
                      <td><span className={`status-badge ${order.status}`}>{order.status}</span></td>
                      <td><button className="action-small view" onClick={() => openDetailModal("orderDetail", order, "order")}>View</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {showUserModal && (
        <div className="modal-overlay">
          <div className="modal-content large" ref={modalRefs.user} tabIndex="-1" role="dialog" aria-modal="true" aria-label="All Users">
            <div className="modal-header">
              <h2>👥 All Users</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
      ...
      {showRevenueModal && (
        <div className="modal-overlay">
          <div className="modal-content" ref={modalRefs.revenue} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Revenue Details">
            <div className="modal-header">
              <h2>💵 Revenue Details</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
            <div className="modal-body">
              <div className="stats-detail">
                <div className="detail-item"><label>Total Earned</label><span className="value">{formatCurrency(stats.totalEarned)}</span></div>
                <div className="detail-item"><label>Today's Sales</label><span className="value">{formatCurrency(stats.todaySales)}</span></div>
                <div className="detail-item"><label>Avg. Order Value</label><span className="value">{formatCurrency(stats.totalOrders ? stats.totalRevenue / stats.totalOrders : 0)}</span></div>
                <div className="detail-item"><label>Total Orders</label><span className="value">{stats.totalOrders}</span></div>
              </div>
              <h4>Revenue by Day (Last 7 Days)</h4>
              <div className="mini-chart">
                {salesData.map((day, i) => (
                  <div key={i} className="chart-bar-container">
                    <div className="chart-bar-label">{day.date}</div>
                    <div className="chart-bar-wrapper">
                      <div className="chart-bar" style={{ width: `${salesData.some(d => d.sales > 0) ? (day.sales / Math.max(...salesData.map(d => d.sales))) * 100 : 0}%`, backgroundColor: "#3498db" }}>{formatCurrency(day.sales)}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showProfitModal && (
        <div className="modal-overlay">
          <div className="modal-content" ref={modalRefs.profit} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Profit & Loss Analysis">
            <div className="modal-header">
              <h2>📈 Profit & Loss</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
            <div className="modal-body">
              <div className="stats-detail">
                <div className="detail-item"><label>Total Earned</label><span className="value">{formatCurrency(stats.totalEarned)}</span></div>
                <div className="detail-item"><label>Total Expenses</label><span className="value loss">{formatCurrency(stats.totalExpenses)}</span></div>
                <div className="detail-item"><label>Net {isNetPositive ? "Profit" : "Loss"}</label><span className={`value ${isNetPositive ? "profit" : "loss"}`}>{isNetPositive ? "+" : "-"}{formatCurrency(Math.abs(stats.netProfit))}</span></div>
                <div className="detail-item"><label>Profit Margin</label><span className="value">{stats.totalEarned > 0 ? ((stats.netProfit / stats.totalEarned) * 100).toFixed(1) : 0}%</span></div>
              </div>
              <div className="pnl-summary">
                <div className="pnl-row earned"><span>💵 Money Earned</span><span>{formatCurrency(stats.totalEarned)}</span></div>
                <div className="pnl-row loss"><span>💸 Money Spent</span><span>- {formatCurrency(stats.totalExpenses)}</span></div>
                <div className={`pnl-row ${isNetPositive ? "net-profit" : "net-loss"}`}><span><strong>= Net {isNetPositive ? "Profit" : "Loss"}</strong></span><span><strong>{isNetPositive ? "+" : "-"}{formatCurrency(Math.abs(stats.netProfit))}</strong></span></div>
              </div>
              <h4>Most Profitable Products</h4>
              <div className="profitable-products">
                {topProducts.map((product, i) => {
      ...
      {showFinancialModal && (
        <div className="modal-overlay">
          <div className="modal-content large" ref={modalRefs.financial} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Financial Report">
            <div className="modal-header">
              <h2>💰 Financial Report</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
            <div className="modal-body">
              <div className="financial-summary">
                <div className="fin-card earned"><div className="fin-icon">💵</div><div><p className="fin-label">Total Earned</p><p className="fin-value">{formatCurrency(stats.totalEarned)}</p><p className="fin-sub">{stats.totalOrders} orders</p></div></div>
                <div className="fin-card expenses"><div className="fin-icon">💸</div><div><p className="fin-label">Total Expenses</p><p className="fin-value">{formatCurrency(stats.totalExpenses)}</p><p className="fin-sub">Product costs</p></div></div>
                <div className={`fin-card ${isNetPositive ? "profit" : "loss"}`}><div className="fin-icon">{isNetPositive ? "📈" : "📉"}</div><div><p className="fin-label">Net {isNetPositive ? "Profit" : "Loss"}</p><p className="fin-value">{isNetPositive ? "+" : "-"}{formatCurrency(Math.abs(stats.netProfit))}</p><p className="fin-sub">{isNetPositive ? "Profitable ✅" : "Loss ⚠️"}</p></div></div>
              </div>
              <h4>📋 Expense Ledger</h4>
              <table className="modal-table">
                <thead><tr><th>Date</th><th>Type</th><th>Description</th><th>Amount</th></tr></thead>
                <tbody>
                  {expenses.length === 0 ? <tr><td colSpan="4" className="no-data">No expenses recorded yet.</td></tr> : expenses.map(exp => (
                    <tr key={exp.id}>
                      <td>{new Date(exp.date).toLocaleDateString()}</td>
                ...
                <tfoot><tr><td colSpan="3" style={{ textAlign: "right", fontWeight: "bold" }}>Total:</td><td className="loss-text"><strong>{formatCurrency(stats.totalExpenses)}</strong></td></tr></tfoot>
              </table>
            </div>
            <div className="modal-footer">
              <button className="btn-secondary" onClick={closeAllModals}>Close</button>
      ...
      {showInventoryModal && (
        <div className="modal-overlay">
          <div className="modal-content large" ref={modalRefs.inventory} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Inventory Management">
            <div className="modal-header">
              <h2>📦 Inventory</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
                ...
                <tbody>
                  {products.map((product) => (
                    <tr key={product.id}>
                      ...
                      <td>{product.SKU || `SKU-${product.id}`}</td>
                      <td className={product.stock_quantity < 10 ? "stock-low" : ""}>{product.stock_quantity}</td>
                      <td>{product.stock_quantity < 5 ? "🔴 Critical" : product.stock_quantity < 10 ? "🟡 Warning" : "🟢 Good"}</td>
                      <td className="action-cell">
                        <button className="action-small view" onClick={() => openDetailModal("inventoryDetail", product, "inventory")}>View</button>
                        <button className="action-small" onClick={() => openDetailModal("restock", product, "inventory")}>Restock</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {showPendingModal && (
        <div className="modal-overlay">
          <div className="modal-content large" ref={modalRefs.pending} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Pending Orders">
            <div className="modal-header">
              <h2>⏳ Pending Orders</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
            <div className="modal-body">
              {orders.filter(o => o.status === "pending").length === 0 ? (
                ...
                <table className="modal-table">
                  <thead><tr><th>Order ID</th><th>Customer</th><th>Email</th><th>Date</th><th>Total</th><th>Update Status</th><th>Detail</th></tr></thead>
                  <tbody>
                    {orders.filter(o => o.status === "pending").map((order) => (
                      <tr key={order.id}>
                        ...
                        <td>{formatCurrency(order.total)}</td>
                        <td>
                          <select value={order.status} onChange={(e) => { handleUpdateOrderStatus(order.id, e.target.value); }} className="status-select">
                            <option value="pending">Pending</option>
                            <option value="processing">Processing</option>
                            <option value="shipped">Shipped</option>
                            <option value="completed">Completed</option>
                        ...
                        <td><button className="action-small view" onClick={() => openDetailModal("orderDetail", order, "pending")}>View</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {showSalesModal && (
        <div className="modal-overlay">
          <div className="modal-content large" ref={modalRefs.sales} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Sales Analysis">
            <div className="modal-header">
              <h2>📊 Sales Analysis</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
            <div className="modal-body">
              <div className="stats-detail">
                <div className="detail-item"><label>Today's Sales</label><span className="value">{formatCurrency(stats.todaySales)}</span></div>
                <div className="detail-item"><label>7-Day Total</label><span className="value">{formatCurrency(salesData.reduce((s, d) => s + d.sales, 0))}</span></div>
                <div className="detail-item"><label>7-Day Profit</label><span className="value profit">{formatCurrency(salesData.reduce((s, d) => s + d.profit, 0))}</span></div>
                <div className="detail-item"><label>7-Day Orders</label><span className="value">{salesData.reduce((s, d) => s + d.orders, 0)}</span></div>
              </div>
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={salesData}>
                  <CartesianGrid strokeDasharray="3 3" />
              ...
              <h4>Daily Breakdown</h4>
              <table className="modal-table">
                <thead><tr><th>Day</th><th>Orders</th><th>Sales</th><th>Profit</th></tr></thead>
                <tbody>
                  {salesData.map((day, i) => (
                    <tr key={i}>
                      <td>{day.date}</td>
      ...
      {showProductDetailModal && selectedItem && (
        <div className="modal-overlay">
          <div className="modal-content" ref={modalRefs.productDetail} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Product Details">
            <div className="modal-header">
              <h2>📦 Product Details</h2>
              <button className="modal-close" onClick={closeDetailModal}>×</button>
            </div>
            <div className="modal-body">
              <div className="detail-view">
                ...
                <div className="detail-row"><label>Category ID:</label><span>{selectedItem.category_id}</span></div>
                <div className="detail-row"><label>Price:</label><span>{formatCurrency(selectedItem.price)}</span></div>
                <div className="detail-row"><label>Cost:</label><span>{formatCurrency(selectedItem.cost || selectedItem.price * 0.6)}</span></div>
                <div className="detail-row"><label>Profit/Unit:</label><span className="profit">{formatCurrency(selectedItem.price - (selectedItem.cost || selectedItem.price * 0.6))}</span></div>
                <div className="detail-row"><label>Current Stock:</label><span className={selectedItem.stock_quantity < 10 ? "stock-low" : ""}>{selectedItem.stock_quantity} units</span></div>
                <div className="detail-row"><label>SKU:</label><span>{selectedItem.SKU || `SKU-${selectedItem.id}`}</span></div>
                <div className="detail-row"><label>Description:</label><span>{selectedItem.description || "No description"}</span></div>
                {selectedItem.sizes && selectedItem.sizes.length > 0 && (
                  <div className="detail-row"><label>Sizes:</label><ul>{selectedItem.sizes.map((size, idx) => <li key={idx}>{size}: {selectedItem.availableSizes?.[size] || 0} in stock</li>)}</ul></div>
                )}
                {selectedItem.image && (
                  <div className="detail-row">
                    <label>Image:</label>
                    <img src={selectedItem.image} alt={selectedItem.name} style={{ maxWidth: "200px", maxHeight: "200px", objectFit: "contain" }} onError={(e) => { e.target.onerror = null; e.target.src = "https://via.placeholder.com/200?text=No+Image"; }} />
                  </div>
                )}
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn-primary" onClick={() => { closeDetailModal(); openEditModal(selectedItem, parentModal); }}>✏️ Edit</button>
              <button className="btn-warning" onClick={() => { closeDetailModal(); openDetailModal("restock", selectedItem, parentModal); }}>📦 Restock</button>
      ...
      {showOrderDetailModal && selectedItem && (
        <div className="modal-overlay">
          <div className="modal-content" ref={modalRefs.orderDetail} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Order Details">
            <div className="modal-header">
              <h2>🛒 Order Details</h2>
              <button className="modal-close" onClick={closeDetailModal}>×</button>
            </div>
            <div className="modal-body">
              <div className="detail-view">
                <div className="detail-row"><label>Customer:</label><span>{selectedItem.customer}</span></div>
                <div className="detail-row"><label>Email:</label><span>{selectedItem.email}</span></div>
                <div className="detail-row"><label>Date:</label><span>{new Date(selectedItem.date).toLocaleDateString()}</span></div>
                <div className="detail-row"><label>Status:</label><span className={`status-badge ${selectedItem.status}`}>{selectedItem.status}</span></div>
                <div className="detail-row"><label>Payment:</label><span>{selectedItem.paymentMethod}</span></div>
                <div className="detail-row"><label>Shipping:</label><span>{selectedItem.shippingAddress}</span></div>
              </div>
              <h4>Items Ordered:</h4>
              <table className="items-table">
                ...
                <tbody>
                  {selectedItem.items?.map((item, i) => (
                    <tr key={i}><td>{item.name}</td><td>{formatCurrency(item.price)}</td><td>{item.quantity}</td><td>{formatCurrency(item.price * item.quantity)}</td></tr>
                  ))}
                </tbody>
                <tfoot><tr><td colSpan="3" style={{ textAlign: "right", fontWeight: "bold" }}>Total:</td><td style={{ fontWeight: "bold", color: "#2ecc71" }}>{formatCurrency(selectedItem.total)}</td></tr></tfoot>
              </table>
              <div className="status-update">
                <label>Update Status:</label>
                <select value={selectedItem.status} onChange={(e) => { handleUpdateOrderStatus(selectedItem.id, e.target.value); setSelectedItem({ ...selectedItem, status: e.target.value }); }} className="status-select">
                  <option value="pending">⏳ Pending</option>
                  <option value="processing">⚙️ Processing</option>
                  <option value="shipped">🚚 Shipped</option>
                  <option value="completed">✅ Completed</option>
      ...
      {showInventoryDetailModal && selectedItem && (
        <div className="modal-overlay">
          <div className="modal-content" ref={modalRefs.inventoryDetail} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Inventory Detail">
            <div className="modal-header">
              <h2>📦 Inventory Detail</h2>
              <button className="modal-close" onClick={closeDetailModal}>×</button>
            </div>
            <div className="modal-body">
              <div className={`inventory-alert-banner ${selectedItem.alertLevel}`}>
                {selectedItem.alertLevel === "critical" ? "🔴 CRITICAL" : selectedItem.alertLevel === "warning" ? "🟡 WARNING" : "🔵 NOTICE"}
              </div>
              <div className="detail-view">
                <div className="detail-row"><label>Product:</label><span>{selectedItem.name}</span></div>
                <div className="detail-row"><label>SKU:</label><span>{selectedItem.SKU || `SKU-${selectedItem.id}`}</span></div>
                <div className="detail-row"><label>Category:</label><span>{selectedItem.category_id}</span></div>
                <div className="detail-row"><label>Current Stock:</label><span className="stock-low">{selectedItem.stock_quantity} units</span></div>
              ...
              <div className="stock-bar large-bar">
                <div className="stock-fill" style={{ width: `${Math.min((selectedItem.stock_quantity / 50) * 100, 100)}%`, backgroundColor: selectedItem.alertLevel === "critical" ? "#e74c3c" : selectedItem.alertLevel === "warning" ? "#f39c12" : "#3498db" }} />
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn-primary" onClick={() => { closeDetailModal(); openDetailModal("restock", selectedItem, parentModal); }}>📦 Restock Now</button>
              <button className="btn-secondary" onClick={closeDetailModal}>Close</button>
            </div>
          </div>
        </div>
      ...
      {showAddProductModal && (
        <div className="modal-overlay">
          <div className="modal-content" ref={modalRefs.addProduct} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Add New Product">
            <div className="modal-header">
              <h2>➕ Add New Product</h2>
              <button className="modal-close" onClick={closeAllModals}>×</button>
            </div>
            <div className="modal-body">
              <div className="expense-notice">💸 <strong>Note:</strong> Adding a product records a business expense.</div>
              <form className="product-form" onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="form-group"><label>Category ID *</label><input type="number" value={newProduct.category_id} onChange={(e) => setNewProduct({ ...newProduct, category_id: e.target.value })} required /></div>
                <div className="form-group"><label>Subcategory ID</label><input type="number" value={newProduct.subcategory_id} onChange={(e) => setNewProduct({ ...newProduct, subcategory_id: e.target.value })} /></div>
                <div className="form-group"><label>SKU *</label><input type="text" value={newProduct.SKU} onChange={(e) => setNewProduct({ ...newProduct, SKU: e.target.value })} required /></div>
                <div className="form-row">
                  <div className="form-group"><label>Sell Price ($) *</label><input type="number" step="0.01" min="0" value={newProduct.price} onChange={(e) => setNewProduct({ ...newProduct, price: e.target.value })} required /></div>
                  <div className="form-group"><label>Cost Price ($) *</label><input type="number" step="0.01" min="0" value={newProduct.cost} onChange={(e) => setNewProduct({ ...newProduct, cost: e.target.value })} required /></div>
                </div>
                <div className="form-row">
                  <div className="form-group"><label>Initial Stock *</label><input type="number" min="0" value={newProduct.stock_quantity} onChange={(e) => setNewProduct({ ...newProduct, stock_quantity: e.target.value })} required /></div>
                ...
                <div className="form-group"><label>Description *</label><textarea value={newProduct.description} onChange={(e) => setNewProduct({ ...newProduct, description: e.target.value })} rows="3" required /></div>
                <div className="form-group sizes-section">
                  <label>📏 Sizes & Stock (optional)</label>
                  <p className="sizes-help">Add sizes if this product comes in multiple sizes. Leave empty for products without variants.</p>
                  {newProduct.sizes.map((entry, index) => (
                    <div key={index} className="size-row">
                      <input type="text" placeholder="Size (e.g., S, M, L)" value={entry.size} onChange={(e) => { const updated = [...newProduct.sizes]; updated[index].size = e.target.value; setNewProduct({ ...newProduct, sizes: updated }); }} />
                      <input type="number" min="0" placeholder="Stock" value={entry.stock_quantity} onChange={(e) => { const updated = [...newProduct.sizes]; updated[index].stock_quantity = parseInt(e.target.value) || 0; setNewProduct({ ...newProduct, sizes: updated }); }} />
                      <button type="button" className="remove-size-btn" onClick={() => { const updated = newProduct.sizes.filter((_, i) => i !== index); setNewProduct({ ...newProduct, sizes: updated }); }} aria-label="Remove size">✕</button>
                    </div>
                  ))}
                  <button type="button" className="add-size-btn" onClick={() => { setNewProduct({ ...newProduct, sizes: [...newProduct.sizes, { size: "", stock_quantity: 0 }] }); }}>+ Add Size</button>
                </div>
                {newProduct.cost && newProduct.stock_quantity && (
                  <div className="expense-preview">
                    <strong>📊 Investment Summary:</strong>
                    <div>Total Cost: {formatCurrency(parseFloat(newProduct.cost || 0) * parseInt(newProduct.stock_quantity || 0))}</div>
                    <div>Expected Revenue: {formatCurrency(parseFloat(newProduct.price || 0) * parseInt(newProduct.stock_quantity || 0))}</div>
                    <div>Potential Profit: {formatCurrency((parseFloat(newProduct.price || 0) - parseFloat(newProduct.cost || 0)) * parseInt(newProduct.stock_quantity || 0))}</div>
                  </div>
                )}
              </form>
            </div>
            <div className="modal-footer">
              <button className="btn-primary" onClick={handleAddProduct} disabled={!newProduct.name || !newProduct.category_id || !newProduct.price || !newProduct.cost || !newProduct.stock_quantity || !newProduct.SKU || !newProduct.description}>Add Product</button>
      ...
      {showEditProductModal && selectedItem && (
        <div className="modal-overlay">
          <div className="modal-content" ref={modalRefs.editProduct} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Edit Product">
            <div className="modal-header">
              <h2>✏️ Edit Product</h2>
              <button className="modal-close" onClick={closeDetailModal}>×</button>
            </div>
            <div className="modal-body">
              <form className="product-form" onSubmit={(e) => e.preventDefault()}>
                <div className="form-group"><label>Product Name *</label><input type="text" value={editProduct.name} onChange={(e) => setEditProduct({ ...editProduct, name: e.target.value })} required /></div>
                <div className="form-group"><label>Category ID *</label><input type="number" value={editProduct.category_id} onChange={(e) => setEditProduct({ ...editProduct, category_id: e.target.value })} required /></div>
                <div className="form-group"><label>Subcategory ID</label><input type="number" value={editProduct.subcategory_id || ""} onChange={(e) => setEditProduct({ ...editProduct, subcategory_id: e.target.value })} /></div>
                <div className="form-group"><label>SKU *</label><input type="text" value={editProduct.SKU} onChange={(e) => setEditProduct({ ...editProduct, SKU: e.target.value })} required /></div>
                <div className="form-row">
                  <div className="form-group"><label>Sell Price ($) *</label><input type="number" step="0.01" min="0" value={editProduct.price} onChange={(e) => setEditProduct({ ...editProduct, price: e.target.value })} required /></div>
                  <div className="form-group"><label>Cost Price ($) *</label><input type="number" step="0.01" min="0" value={editProduct.cost} onChange={(e) => setEditProduct({ ...editProduct, cost: e.target.value })} required /></div>
                </div>
                <div className="form-row">
                  <div className="form-group"><label>Stock *</label><input type="number" min="0" value={editProduct.stock_quantity} onChange={(e) => setEditProduct({ ...editProduct, stock_quantity: e.target.value })} required /></div>
                  <div className="form-group"><label>Image URL</label><input type="text" value={editProduct.image || ""} onChange={(e) => setEditProduct({ ...editProduct, image: e.target.value })} /></div>
                </div>
                <div className="form-group"><label>Description *</label><textarea value={editProduct.description} onChange={(e) => setEditProduct({ ...editProduct, description: e.target.value })} rows="3" required /></div>
                <div className="form-group sizes-section">
                  <label>📏 Sizes & Stock</label>
                  <p className="sizes-help">Modify sizes and their available stock.</p>
                  {editProduct.sizes.map((entry, index) => (
                    <div key={index} className="size-row">
                      <input type="text" placeholder="Size (e.g., S, M, L)" value={entry.size} onChange={(e) => { const updated = [...editProduct.sizes]; updated[index].size = e.target.value; setEditProduct({ ...editProduct, sizes: updated }); }} />
                      <input type="number" min="0" placeholder="Stock" value={entry.stock_quantity} onChange={(e) => { const updated = [...editProduct.sizes]; updated[index].stock_quantity = parseInt(e.target.value) || 0; setEditProduct({ ...editProduct, sizes: updated }); }} />
                      <button type="button" className="remove-size-btn" onClick={() => { const updated = editProduct.sizes.filter((_, i) => i !== index); setEditProduct({ ...editProduct, sizes: updated }); }} aria-label="Remove size">✕</button>
                    </div>
                  ))}
                  <button type="button" className="add-size-btn" onClick={() => { setEditProduct({ ...editProduct, sizes: [...editProduct.sizes, { size: "", stock_quantity: 0 }] }); }}>+ Add Size</button>
                </div>
              </form>
            </div>
            <div className="modal-footer">
      ...
      {showRestockModal && selectedItem && (
        <div className="modal-overlay">
          <div className="modal-content small" ref={modalRefs.restock} tabIndex="-1" role="dialog" aria-modal="true" aria-label="Restock Product">
            <div className="modal-header">
              <h2>📦 Restock Product</h2>
              <button className="modal-close" onClick={closeDetailModal}>×</button>
            </div>
            <div className="modal-body">
              ...
              <p><strong>Current Stock:</strong> {selectedItem.stock_quantity} units</p>
              <p><strong>Cost per unit:</strong> {formatCurrency(selectedItem.cost || selectedItem.price * 0.6)}</p>
              <div className="form-group">
                <label>Quantity to Add:</label>
                <input type="number" min="1" value={restockAmount[selectedItem.id] || ""} onChange={(e) => setRestockAmount({ ...restockAmount, [selectedItem.id]: parseInt(e.target.value) || 0 })} placeholder="Enter quantity" />
              </div>
              {restockAmount[selectedItem.id] > 0 && (
                <div className="expense-preview">💸 Restock expense: <strong>{formatCurrency((selectedItem.cost || selectedItem.price * 0.6) * restockAmount[selectedItem.id])}</strong></div>
              )}
            </div>
            <div className="modal-footer">
              <button className="btn-primary" onClick={() => handleRestock(selectedItem.id, restockAmount[selectedItem.id] || 0)} disabled={!restockAmount[selectedItem.id] || restockAmount[selectedItem.id] < 1}>Confirm Restock</button>
              <button className="btn-secondary" onClick={closeDetailModal}>Cancel</button>
            </div>
          </div>
        </div>
